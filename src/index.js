...
module.exports = () => [
  ...
  },
  {
    ...
    filter: text => text.replace(
      /^\[\^([\d\w]+)\]:( |\n)((.+\n)*.+)$/mg,
      (str, name, _, content) =>
        `<small class="footnote" id="footnote-${name}"><a href="#footnote-ref-${name}"><sup>[${name}]</sup></a>: ${content}</small>`
    )
  },
  {
    type: 'lang',
    filter: text => text.replace(
      /\[\^([\d\w]+)\]/mg,
      (str, name) => `<a id="footnote-ref-${name}" href="#footnote-${name}"><sup>[${name}]</sup></a>`
    )
  }
]
